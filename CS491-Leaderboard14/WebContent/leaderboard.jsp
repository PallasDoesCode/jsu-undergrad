...
<%@include file="leaderboard_student.jsp" %>
	...
	%>		
		...
		<script type="text/javascript">		
			...
			<% B2Context bc = new B2Context(request); %>
			
			// Get the session user's current score.
			var sessionUserScore = { current: <% out.print(scoreToHighlight); %> };
			
			// Returns the previously saved session user's score from the userID_prevScore cookie.
			function getSessionUserPrevScore()
			{
				var prevScore = parseFloat(<% out.print(bc.getSetting(true, true, sessionUserID + "previousScore")); %>);
				if(isNaN(prevScore))
				{
					return sessionUserScore.current;
				}
				else
				{
					return prevScore;
			...
			// Gets the previous score, the value for the first half of bar, and the value for the second half of bar.
			sessionUserScore.previous = getSessionUserPrevScore();
			sessionUserScore.base = Math.min(sessionUserScore.current, sessionUserScore.previous);
			sessionUserScore.delta = Math.max(sessionUserScore.current - sessionUserScore.previous, 0);
			console.log("previous2: " +sessionUserScore.previous +" current2: " +sessionUserScore.current); // DEBUG
			console.log("base: " +sessionUserScore.base +" delta: " +sessionUserScore.delta); // DEBUG
	        ...
	        dave(document).ready(function() {
				...
				var seriesValues = [
	  				<%	
	  					...
	  					for (int x = 0; x < students.size(); x++){
	  						...
	  						if (score == scoreToHighlight && !alreadyHighlighted) {
	  							...
	  							out.print("{y: sessionUserScore.base, color: '"+ user_color + "'}");
	  						}
	  						else {
	  			...
	  			var seriesValues2 = [ // This is the second values for the stacked bar; it shows points gained
	  				<%
		  				for(int y = 0; y < students.size(); y++){
	  						Double score = (Double) students.get(y).score;
	  						if(score == scoreToHighlight) {
	  							out.print("{y: sessionUserScore.delta, color: 'red'}");
	  						}
	  						else{
	  							out.print("{y: 0, color: '" + color + "'}");
	  						}
	  						if (y < students.size() -1) { out.print(","); }
	  						else { out.print("];"); }
	  					}
	  				%>		                     
 				...
 				gamegogyLeaderboardChart = new Highcharts.Chart({
					...
					tooltip: {
						formatter: function() {
							if (this.series.name == "XP Gained" && this.y == 0){
								return false;	
							}
							else{
								return this.y;
					...
					if(!<% out.print(canUserSeeScores); %>){
						// Draw the points counter
						var value=0, end=sessionUserScore.current;
						var x=0, y=0, h=16, w=200;	// x and y position, height and width				
						var increment = end * .1;	// How much the counter increments each interval
						var rate = 70;				// How many milliseconds between each render
						var intervalID = setInterval(function() {
							chart.renderer.rect(x, y, w, h, 0).css({fill: '#fff'}).add();
							chart.renderer.text("Total score: " +Math.floor(value), x, y+h).css({color: '#000', fontSize: h+'px'}).add();
				...
				// DEBUG
				dave(seriesValues).each(function(index){ console.log("seriesValues[" + index +"]: " + seriesValues[index].y); });
				dave(seriesValues2).each(function(index){ console.log("seriesValues2[" + index +"]: " + seriesValues2[index].y); });

				// Save the session user's score as a cookie named userID_prevScore.
				<%
					bc.setSetting(true, true, sessionUserID + "previousScore", Double.toString(scoreToHighlight));
					bc.persistSettings(true, true);
				%>
